...
router.post("/", async (req, res) => {
    try {
        ...
        if(!id_colore_esterno) {
            return res.status(400).send({ field: "errore_id_colore_esterno", message: "Id 5 colore esterno necessario" });
        }
        if(!id_colore_interno) {
            isAccessorio = true;
        }
        ...
        if(!isAccessorio) {
            await db.execute(
                `INSERT INTO ordini (id_articolo, id_colore_esterno, id_colore_interno, quantita, consegnato, data_consegna, riferimento) VALUES (?,?,?,?,0,?,?)`,
                [id_prodotto, id_colore_esterno, id_colore_interno, quantita, data, riferimento]
            )
        }
        else {
            await db.execute(
                `INSERT INTO ordini (id_accessorio, id_colore_esterno, quantita, consegnato, data_consegna, riferimento) VALUES (?,?,?,0,?,?)`,
                [id_prodotto, id_colore_esterno, quantita, data, riferimento]
            )
        }
...
router.post("/:id", async (req, res) => {
    try {
        ...
        if(!id_colore_esterno) {
            return res.status(400).send({ field: "errore_id_colore_esterno", message: "Id 6 colore esterno necessario" });
        }
        if(!id_colore_interno) {
            isAccessorio = true;
        }
        ...
        if(!isAccessorio) {
            result = await db.execute(
                ...
                [id_prodotto, id_colore_esterno, id_colore_interno, quantita, data_consegna, 0, riferimento, req.params.id]
            )
        }
        else {
            result = await db.execute(
                ...
                [id_prodotto, id_colore_esterno, quantita, data_consegna, 0, riferimento, req.params.id]
            )
        }
...
router.post("/consegna/:id", async (req, res) => {
    try {
        ...
        const result = await db.execute(`UPDATE ordini SET consegnato = 1 WHERE id = ?`, [id]);
        if(result[0].affectedRows <= 0) {
            throw Error();
        }
...
router.post("/elimina/:id", async (req, res) => {
    try {
        ...
        const result = await db.execute("DELETE FROM ordini WHERE id = ?", [id]);
        if(result[0].affectedRows <= 0) {
            return res.status(400).send({ field: "errore_id", message: "Id ordine non presente nel database" });
        }
